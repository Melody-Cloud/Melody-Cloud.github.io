...
let AplikacjaWycieczkiComponent = class AplikacjaWycieczkiComponent {
    constructor(socket, toastr) {
        this.socket = socket;
        this.toastr = toastr;
        this.socket.on('message', (event) => {
            ...
            if (('currentPromotions' in event) && !lodash_isEqual__WEBPACK_IMPORTED_MODULE_4___default()(event.currentPromotions, {})) {
                let message = '';
                lodash_forEach__WEBPACK_IMPORTED_MODULE_5___default()(event.currentPromotions, (value, key) => {
...
AplikacjaWycieczkiComponent.ctorParameters = () => [
...
let AdminPanelComponent = class AdminPanelComponent {
    constructor(authService, router, spinner, firebaseService, promotionService) {
    ...
    addExampleTrips() {
        _data_fake_dane__WEBPACK_IMPORTED_MODULE_5__["fakeWycieczki"].forEach((wycieczka) => {
    ...
    refreshTrips() {
        this.spinner.show();
        this.firebaseService.getAllTrips().subscribe((products) => {
            this.wycieczki = products;
            products.forEach(wycieczka => {
                this.checkedTripsForPromotion[wycieczka[src_app_const__WEBPACK_IMPORTED_MODULE_7__["IDKEY"]]] = false;
            });
            this.spinner.hide();
        });
    }
    removeTrip(wycieczka) {
        const observable = this.firebaseService.deleteTrip(wycieczka);
    }
    addPromotion() {
        const checkedTrips = lodash_filter__WEBPACK_IMPORTED_MODULE_10___default()(lodash_map__WEBPACK_IMPORTED_MODULE_9___default()(this.checkedTripsForPromotion, (value, key) => {
...
let AfterOrderComponent = class AfterOrderComponent {
    constructor(route) {
        this.route = route;
    }
    ngOnInit() {
...
AfterOrderComponent.ctorParameters = () => [
...
let DropFileComponent = class DropFileComponent {
...
DropFileComponent.ctorParameters = () => [
...
let EditWycieczkaComponent = class EditWycieczkaComponent {
    constructor(wycieczkiService, firebaseService, route, spinner) {
    ...
    ngOnInit() {
        ...
        this.spinner.show();
        this.firebaseService.getTrip(wycieczkaId).subscribe((wycieczka) => {
            this.modelForm = new _angular_forms__WEBPACK_IMPORTED_MODULE_2__["FormGroup"]({
    ...
    onSubmit(form) {
        const newWycieczka = Object.assign({}, form.value, { [_const__WEBPACK_IMPORTED_MODULE_7__["IDKEY"]]: this.wycieczkaId });
        this.firebaseService.updateTrip(newWycieczka);
    }
};
EditWycieczkaComponent.ctorParameters = () => [
...
let HeaderComponent = class HeaderComponent {
    constructor(authService, fireAuth, firebaseService, router, spinner) {
        this.authService = authService;
    ...
    checkIsUserAnAdministrator() {
        this.fireAuth.auth.onAuthStateChanged((user) => {
            this.firebaseService.getRole(user.email).subscribe(roles => {
                ...
                this.isAdmin = (lodash_get__WEBPACK_IMPORTED_MODULE_6___default()(roles, '[0].role') === 'admin');
            });
        });
    }
};
...
let KoszykComponent = class KoszykComponent {
...
KoszykComponent.ctorParameters = () => [
...
let ListaWycieczekComponent = class ListaWycieczekComponent {
    constructor(koszykService, wycieczkiService, firebaseService, spinner, socket, toastr) {
        this.koszykService = koszykService;
        this.wycieczkiService = wycieczkiService;
        this.firebaseService = firebaseService;
        this.spinner = spinner;
    ...
    ngOnInit() {
        ...
        this.firebaseService.getAllTrips().subscribe((products) => {
            ...
            this.filterForm = new _angular_forms__WEBPACK_IMPORTED_MODULE_4__["FormGroup"]({
                ...
                priceMin: new _angular_forms__WEBPACK_IMPORTED_MODULE_4__["FormControl"](this.minPriceTrip.cenaJednostkowa),
                priceMax: new _angular_forms__WEBPACK_IMPORTED_MODULE_4__["FormControl"](this.maxPriceTrip.cenaJednostkowa),
...
ListaWycieczekComponent.ctorParameters = () => [
    { type: _services_koszyk_service__WEBPACK_IMPORTED_MODULE_2__["KoszykService"] },
    { type: _services_wycieczki_serwis_service__WEBPACK_IMPORTED_MODULE_3__["WycieczkiSerwisService"] },
    { type: _services_firebase_service__WEBPACK_IMPORTED_MODULE_5__["FirebaseService"] },
    { type: ngx_spinner__WEBPACK_IMPORTED_MODULE_6__["NgxSpinnerService"] },
...
let NewWycieczkaComponent = class NewWycieczkaComponent {
    constructor(wycieczkiService, firebaseService, fileUploadService) {
        this.wycieczkiService = wycieczkiService;
        this.firebaseService = firebaseService;
        this.fileUploadService = fileUploadService;
    }
    ...
    onSubmit(form) {
        const newWycieczka = Object.assign({}, form.value, { ileZarezerwowano: 0, oceny: [], galleryImgs: this.fileUploadService.getFilenames() });
        this.fileUploadService.reset();
        this.firebaseService.addTrip(newWycieczka);
    }
};
NewWycieczkaComponent.ctorParameters = () => [
    { type: _services_wycieczki_serwis_service__WEBPACK_IMPORTED_MODULE_3__["WycieczkiSerwisService"] },
    { type: _services_firebase_service__WEBPACK_IMPORTED_MODULE_4__["FirebaseService"] },
...
let OcenaComponent = class OcenaComponent {
    constructor() {
...
let ViewMyOrdersComponent = class ViewMyOrdersComponent {
...
ViewMyOrdersComponent.ctorParameters = () => [
    { type: _services_auth_service__WEBPACK_IMPORTED_MODULE_2__["AuthService"] },
...
let WycieczkaDetalComponent = class WycieczkaDetalComponent {
    constructor(wycieczkiService, koszykService, route, firebaseService, authService, spinner, coordinatesService) {
        this.wycieczkiService = wycieczkiService;
        this.koszykService = koszykService;
        this.route = route;
        this.firebaseService = firebaseService;
    ...
    checkIfPersonReserveTrip(trip) {
        this.firebaseService.getAllOrders().subscribe((orders) => {
            const filteredOrders = orders.filter((order) => {
                if (order.products) {
                    const tripsIds = order.products.map(product => product.trip[_const__WEBPACK_IMPORTED_MODULE_8__["IDKEY"]]);
    ...
    ngOnInit() {
        ...
        this.firebaseService.getTrip(wycieczkaId).subscribe(response => {
            const lonLat = this.coordinatesService.getCoordinates(lodash_get__WEBPACK_IMPORTED_MODULE_10___default()(response, 'docelowyKrajWycieczki'));
            this.lon = lonLat[0];
            this.lat = lonLat[1];
            this.initializeMap();
...
WycieczkaDetalComponent.ctorParameters = () => [
...
let WycieczkaComponent = class WycieczkaComponent {
...
WycieczkaComponent.ctorParameters = () => [
...
const IDKEY = 'id';
const IS_REST = false;
const GALLERY_UPLOAD_URL = 'http://ec2-18-184-171-99.eu-central-1.compute.amazonaws.com:5052/upload';
const GALLERY_SERVE_URL = 'http://ec2-18-184-171-99.eu-central-1.compute.amazonaws.com:5052/get';
// export const GALLERY_UPLOAD_URL = 'http://localhost:5052/upload';
// export const GALLERY_SERVE_URL = 'http://localhost:5052/get';
const PROMOTION_API_ENDPOINT = 'registerPromotion';
// export const PROMOTION_API_URL = 'http://localhost:8080/';
// export const SOCKET_IO_URL = 'http://localhost:8080/';
const PROMOTION_API_URL = 'http://ec2-18-184-171-99.eu-central-1.compute.amazonaws.com:8080/';
const SOCKET_IO_URL = 'http://ec2-18-184-171-99.eu-central-1.compute.amazonaws.com:8080/';
...
let AdminAuthGuard = class AdminAuthGuard {
    constructor(authService, router, firebaseService, fireAuth) {
        this.authService = authService;
        this.router = router;
        this.firebaseService = firebaseService;
        this.fireAuth = fireAuth;
...
let LoggedInAuthGuard = class LoggedInAuthGuard {
    ...
    canActivate(next, state) {
        return this.authService.authState$.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])(state => {
            if (state !== null) {
                return true;
            }
            this.router.navigate(['/login']);
...
let EquityFilterPipe = class EquityFilterPipe {
...
let AuthService = class AuthService {
    ...
    login({ email, password }) {
        if (this.fireAuth.user != null) {
            return this.fireAuth.auth.setPersistence(firebase_app__WEBPACK_IMPORTED_MODULE_4__["auth"].Auth.Persistence.SESSION).then(() => {
...
let FileUploadService = class FileUploadService {
    ...
    getFilenames() {
        ...
        return this.files.map((fileObj) => fileObj.fileEntry.file(file => file.name));
    }
    reset() {
        this.files = [];
    }
            ...
            if (droppedFile.fileEntry.isFile) {
                ...
                fileEntry.file((file) => {
                    ...
                    reader.onload = function () {
                        client.post(`${_const__WEBPACK_IMPORTED_MODULE_2__["GALLERY_UPLOAD_URL"]}/${file.name}`, file).subscribe(results => {
                            console.dir(results);
...
let FirebaseService = class FirebaseService {
    ...
    addOrder(order) {
        lodash_forEach__WEBPACK_IMPORTED_MODULE_5___default()(lodash_get__WEBPACK_IMPORTED_MODULE_4___default()(order, 'products'), product => {
            this.partialUpdate(product.trip[_const__WEBPACK_IMPORTED_MODULE_6__["IDKEY"]], { ileZarezerwowano: product.trip.ileZarezerwowano });
        });
        const pushId = this.db.createPushId();
        order.id = pushId;
        console.log(pushId);
    ...
    getRole(email) {
        console.log(email);
        return this.db.list('/roles', (ref) => ref.orderByChild('email').equalTo(email)).valueChanges();
    }
};
...
let KoszykService = class KoszykService {
    constructor(firebaseService, spinner, authService, router) {
        this.firebaseService = firebaseService;
        this.spinner = spinner;
        this.authService = authService;
        this.router = router;
    ...
    addProduct(product, preferredDate = new Date()) {
        console.dir(product);
        let foundProduct = this.produkty.filter(item => item.trip[_const__WEBPACK_IMPORTED_MODULE_6__["IDKEY"]] === product[_const__WEBPACK_IMPORTED_MODULE_6__["IDKEY"]]);
...
let LonLatCoordinatesService = class LonLatCoordinatesService {
    constructor() {
...
let PromotionService = class PromotionService {
    ...
    addPromotion(checkedTripsForPromotion, czasTrwania, poziomObnizki) {
        ...
        return this.http.post(`${_const__WEBPACK_IMPORTED_MODULE_3__["PROMOTION_API_URL"]}${_const__WEBPACK_IMPORTED_MODULE_3__["PROMOTION_API_ENDPOINT"]}`, objToPost).subscribe(resp => {
            console.log(resp);
        });
    }
};
...
let WycieczkiSerwisService = class WycieczkiSerwisService {
    constructor(http) {
        this.http = http;
        this.fetchProducts();
    }
...
WycieczkiSerwisService.ctorParameters = () => [
    { type: _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpClient"] }
];
